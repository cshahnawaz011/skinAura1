...
import {
  ...
  Download, ChevronLeft, ChevronRight, Loader2, Plus
} from 'lucide-react';
...
export default function Progress() {
  ...
  const uploadMutation = useMutation({
    mutationFn: async (data) => {
      ...
      // Get latest score from analysis
      const latestScore = analyses.length > 0 
        ? analyses[analyses.length - 1].overall_score 
        : 50;
      ...
      <div className="max-w-4xl mx-auto">
        <GlassCard className="text-center py-12">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-6">
            Sign in to track your skin transformation over time
          </p>
          <Button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <GlassCard delay={0.1}>
          <div className="text-center">
            ...
            <CircularProgress
              value={analyses[analyses.length - 1]?.overall_score || 0}
              size={80}
              strokeWidth={8}
              color="pink"
            />
            ...
            {improvement ? (
              <div className="flex items-center justify-center gap-2">
                ...
                <span className={`text-3xl font-bold ${
                  ...
                  improvement.direction === 'down' ? 'text-red-500' : 'text-gray-400'
                }`}>
        ...
        <GlassCard>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm text-gray-500">
              Comparing Week 1 to Week {compareIndex + 2}
            </span>
            <Button
              variant="ghost"
              size="icon"
              onClick={() => setCompareIndex(Math.min(progressPhotos.length - 2, compareIndex + 1))}
              disabled={compareIndex >= progressPhotos.length - 2}
            >
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="text-center">
              ...
              <p className="mt-2 font-medium">
                Score: {progressPhotos[progressPhotos.length - 1]?.skin_score}
              </p>
            </div>
            <div className="text-center">
              <p className="text-sm text-gray-500 mb-2">After (Week {compareIndex + 2})</p>
              <img
                src={progressPhotos[progressPhotos.length - 2 - compareIndex]?.photo_url}
                alt="After"
                className="w-full h-64 object-cover rounded-xl"
              />
              <p className="mt-2 font-medium">
                Score: {progressPhotos[progressPhotos.length - 2 - compareIndex]?.skin_score}
              </p>
            </div>
          </div>
        </GlassCard>
            ...
            <GlassCard>
              ...
              <div className="flex gap-3">
                ...
                <Button
                  ...
                  disabled={!uploadData.photo || uploading}
      ...
      {progressPhotos.length === 0 && !isLoading && (
        <GlassCard className="text-center py-12">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-4">
            Take weekly photos to track your skin transformation
          </p>
          <Button
            onClick={() => setShowUpload(true)}
            className="bg-gradient-to-r from-pink-500 to-amber-500"
